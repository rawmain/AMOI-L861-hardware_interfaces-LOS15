...
static int mMinundeqBuffers = 3;

int CameraDevice::sSetBufferCount(struct preview_stream_ops* w, int count) {
    CameraPreviewWindow* object = static_cast<CameraPreviewWindow*>(w); 
    
    mMinundeqBuffers = count;  
    if (object->mPreviewCallback == nullptr) {
		#ifdef MTK_HARDWARE
        ALOGW("%s: MTK fix Preview returns 0", __FUNCTION__);
        return 0;
        #endif 
        ALOGE("%s: camera HAL calling preview ops while there is no preview window!", __FUNCTION__);
        return INVALID_OPERATION;
    ...
    object->mPreviewCallback->setBufferCount(count);
    return 0;
}
...
int CameraDevice::sGetMinUndequeuedBufferCount(
    ...
    if (object->mPreviewCallback == nullptr||count == nullptr) {
        #ifdef MTK_HARDWARE
        ALOGW("%s: MTK fix Preview returns 0", __FUNCTION__);
        if (count != nullptr) {
			*count = mMinundeqBuffers;
		}
        return 0;
        #endif 
